...
const mockSearchDocuments = mock(() => []);
const mockCreateDocument = mock(() => null);
...
describe("GET /api/documents", () => {
  test("returns 401 when unauthenticated", async () => {
    ...
    const res = await GET(makeReq() as unknown);
  ...
  test("uses FTS search when q param is present", async () => {
    ...
    const res = await GET(makeReq({ q: "hello" }) as unknown);
    const body = await res.json();
    expect(body).toEqual([fakeDoc]);
...
describe("POST /api/documents", () => {
  ...
  test("creates document inside a folder", async () => {
    ...
    const res = await POST(makeReq({}, { folderId: "folder-1" }) as unknown);
    expect(res.status).toBe(201);
